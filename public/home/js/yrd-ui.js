...
//                $input.val('');
      ...
      //数字输入框调节
        f2eUIspinner : function (opts) {
            return this.each(function () {
              var defaults = {value:1, min:0,step2:100}
              var options = $.extend(defaults, opts)
              var keyCodes = {up:38, down:40}
              var container = $('<div></div>')
              container.addClass('spinner clearfix')
              var textField = $(this).addClass('value').val(options.value)
                .bind('blur',function(){
                    validate(textField)
                })
              textField.wrap(container)
              var increaseButton = $('<a class="increase" href="javascript:;">+</a>').click(function () {if(textField.val()<options.value){changeValue(options.step2)} else{changeValue(options.step)} })
              var decreaseButton = $('<a class="decrease" href="javascript:;">-</a>').click(function () { if(textField.val()<=options.value){changeValue(-options.step2)} else{changeValue(-options.step)} })
              //container.data('lastValidValue', options.value)
              textField.before(decreaseButton)
              textField.after(increaseButton)
              textField.css('float','left');
              validateAndTrigger(decreaseButton);
              ...
              function validate(field) {
                ...
                if (value <= options.min) {
                    ...
                    if(value > options.max){
                        $('#'+options.errorplace).text(options.errorTips).show();
...
$(function(){
	  //下面是隐藏和显示表格
	$("body").on('click','.flex_table .flex_tr',function(){
		var self = $(this),
				$next = self.next(),
				$flexA = $(this).find('.flex_a');
        if($flexA.hasClass("flex_plus")&&$next.hasClass("detail_tr")){
        		$flexA.removeClass("flex_plus").addClass("flex_minus");
        		$next.show();
        }else if($flexA.hasClass("flex_minus")&&$next.hasClass("detail_tr")){
        		$flexA.removeClass("flex_minus").addClass("flex_plus");
        		$next.hide();
        }
    });	
    // tab
