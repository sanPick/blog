    ...
    <script>
			...
			function checkRecharge1() {
				...
				$(function(){
					...
					$('.pay').click(function(){
						$(this).attr('width','40').siblings().attr('width','30','height','30')
						$(this).attr('height','40').siblings().attr('height','30','width','30')
						typeflag = $(this).attr('opt')
					})
				})	
			...
			$(document).on('click','#sub',function(){
				...
				var payflag=rechargeFlag;
				var defaultBankName = $("#form\\:defaultBankName").text();

				if(typeof typeflag  == 'undefined' ){
					$('.quick-error3').html("<em></em>请选择支付方式")
					return false
				}
				if(rechargeFlag==false){
					return false
				}
				var _token = $('input[name=_token]').val()
				
				var data = {_token:_token,money:rechargeFlag,pay_type:typeflag}
				if(typeflag==1){
					$.ajax({
						type:'post',
						data:data,
					...
					$.ajax({
						url: "/wechat/businesspay/"+rechargeFlag,
						success: function(msg){
							art.dialog({
								title: '微信支付',
								content: msg,
...
//									return viem('home.paysuc',['money'=>$money]
								}
							});
						}
					});

//					var _url = "<?//=env('APP_URL')?>//"+'/wechat/businesspay/'+rechargeFlag
          ...
          <form id="form" name="form" method="post" action="" >
            ...
            <div class="quick-main">
              <div class="fl quick-info">
                ...
                <div class="info-2"><span class="info-tit" style="padding-bottom:20px;">支付方式</span>
								<span style="margin-top:13px; float:left;"  class="info1-input" >
								<img src="home/images/zhifubao.png" class="pay" id="alipay" opt="1" width="30" heignt="30" style="margin:10px;display:inline-block"/>
								<img src="home/images/weixin.png" id="quick-error3"  class="pay" id="wxpay" opt="2" style="margin:10px;display:inline-block" width="30" heignt="30"/>
								</span> 
							<span class="quick-error3" style="display:inline-block"></span>
								</div>
                ...
                <input type="button" name="" value="充值" class="btn-paycz" id="sub" >
              </div>
